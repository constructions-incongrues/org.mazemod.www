...
class File_XSPF
{
    ...
    function toFile($filename)
    {
        $fp = @fopen($filename, "w");
        if (! $fp) {
            throw new File_XSPF_Exception("Could Not Open File",
                                    File_XSPF::ERROR_FILE_OPENING);
        }
        if (! fwrite($fp, $this->toString())) {
            throw new File_XSPF_Exception("Writing to File Failed",
                                    File_XSPF::ERROR_FILE_WRITING);
        }
        if (! fclose($fp)) {
            throw new File_XSPF_Exception("Failed to Close File",
